...
public class FilterFramework extends Thread
{
	...
	private ArrayList<FilterFramework> InputFilter;

	FilterFramework(int input,int output){
		for(int iterator_input = 0; iterator_input < input;iterator_input++)
			InputReadPort.add(new PipedInputStream());

		for(int iterator_output = 0;iterator_output< output;iterator_output++)
	...
	****************************************************************************/
	...
	public int getAvaliableInputPort(){
		...
		this.numberInputPort++;
		return this.numberInputPort;

	}
	public int getAvaliableOutputPort(){

		this.numberOutputPort++;
		return this.numberOutputPort;
	}
	...
	void Connect( FilterFramework Filter )
	{
		try
		{
			// Connect this filter's input to the upstream pipe's output stream
				...
				InputReadPort.get(getAvaliableInputPort()).connect( Filter.OutputWritePort.get(Filter.getAvailableOutputPort()));
